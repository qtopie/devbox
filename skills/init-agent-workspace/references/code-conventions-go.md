...
- **Single Responsibility & Cohesion**:
  ...
  - **Avoid "Grab-Bag" Packages**: Never create generic `util`, `common`, `helpers`, `models`, or `types` packages. Organize code by domain responsibility (e.g., `crypto`, `auth`, `httputil`, `storage`).
- **Eliminate Stutter (Repetition)**:
  - Do not repeat the package name in exported types, functions, or constants:
    - Correct: `ring.New()`, `http.Client`, `logger.Config`, `bytes.Buffer`
    - Incorrect: `ring.NewRing()`, `http.HttpClient`, `logger.LoggerConfig`, `bytes.ByteBuffer`
